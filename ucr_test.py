...
def get_preterm_data(test_size):
    dataset = PrematureDataset("./data/total_df.csv", 1)

    train, val, test = train_val_test_split(dataset, test_size)
    
    train_loader = DataLoader(train, 4, shuffle=True)
    val_loader = DataLoader(val ,4, shuffle=True)
    test_loader = DataLoader(test ,4, shuffle=True)
...
def train_wo_transfer_learning(test_size):
    train, val, test = get_preterm_data(test_size)
...
# # visualize_seq(train)
# train_loss, val_loss, model = pretrain(train, val, target_size)
# # train, val, test = get_preterm_data("./data/total_df.csv", 0.3)
